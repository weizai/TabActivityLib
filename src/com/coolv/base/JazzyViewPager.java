...
public class JazzyViewPager extends ViewPager {
	...
	private void logState(View v, String title) {
		Log.v(TAG,
				title + ": ROT (" + getRotation() + ", " + getRotationX() + ", " + getRotationY() + "), TRANS (" + getRotationX() + ", " + getTranslationY()
						+ "), SCALE (" + getScaleX() + ", " + getScaleY() + "), ALPHA " + getAlpha());
	}
	...
	protected void animateTablet(View left, View right, float positionOffset) {
		if (mState != State.IDLE) {
			if (left != null) {
				...
				setPivotX(left.getMeasuredWidth() / 2);
				setPivotY(left.getMeasuredHeight() / 2);
